...
router.get('/:uniTag', async (req, res) => {
    ...
    //res.json(faculty);
    const q = `SELECT * FROM faculties WHERE universityTag = '${uniTag}'`
   // const university = await universities.findByPk(uniTag);
    //res.json(university);
    db.query(q, (err, result) => {
        try {
...
router.get('/:uniTag/:facultyName', async (req, res) => {
    ...
    //const faculty = await faculties.findAll({ where: { universityTag: uniTag }});
    //res.json(faculty);
    const q = `SELECT professorName FROM professors WHERE universityTag = '${uniTag}' AND facultyName = '${facultyName}'`
   // const university = await universities.findByPk(uniTag);
    //res.json(university);
    db.query(q, (err, result) => {
        try {
...
router.post('/add-faculty', async(req, res) => {
    ...
    const q = `INSERT INTO faculties(facultyName, universityTag) VALUES ('${post.faculty}', '${post.universityTag}')`
